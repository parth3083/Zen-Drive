...
function Dashboard() {
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get("https://zen-drive-2.onrender.com/files", {
  ...
  const handleLogOut = async () => {
    try {
      const response = await axios.post(
        "https://zen-drive-2.onrender.com/logout",
        {},
        { withCredentials: true }
      );
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const response = await axios.post("https://zen-drive-2.onrender.com/upload", multipleFormData, {
        headers: {
